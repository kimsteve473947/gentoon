...
export async function POST(request: NextRequest) {
  try {
    ...
    // 캐릭터 개수 제한 검증 (ADMIN은 무제한)
    if (planType !== 'ADMIN' && (currentCharacterCount || 0) >= planConfig.maxCharacters) {
      return NextResponse.json(
        { 
          success: false, 
          error: `캐릭터 생성 한도를 초과했습니다 (${currentCharacterCount}/${planConfig.maxCharacters})`,
          needsUpgrade: true,
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 🚀 초고속 캐릭터 조회 - 썸네일만 로딩으로 8.6MB → 26KB 절약
    const url = new URL(request.url);
    const limit = parseInt(url.searchParams.get('limit') || '10');
    const offset = parseInt(url.searchParams.get('offset') || '0');
    
    console.log(`⚡ Loading ${limit} characters with thumbnails only (lightweight mode)`);
