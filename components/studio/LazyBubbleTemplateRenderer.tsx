...
export const LazyBubbleTemplateRenderer = memo(({
  ...
  // SVG 콘텐츠 로드 (새로운 Hook 사용)
  const { svgContent: rawSvgContent, loading } = useBubbleTemplate(template);
  
  // 템플릿 SVG 메모이제이션 및 스타일 적용
  const svgContent = useMemo(() => {
    if (!rawSvgContent || !template) return null;
    ...
    const styledSvg = applySvgStyles(rawSvgContent, fillColor, strokeColor, strokeWidth);
    templateCache.set(cacheKey, styledSvg);
    return styledSvg;
  }, [rawSvgContent, templateId, fillColor, strokeColor, strokeWidth, template]);
