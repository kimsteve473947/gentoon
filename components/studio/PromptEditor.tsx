...
import { MentionTextArea, processMentions } from "./MentionTextArea";
...
export function PromptEditor({ canvasRatio }: PromptEditorProps) {
  ...
  // 멘션용 캐릭터/요소 데이터 변환
  const mentionCharacters = characters.map(char => ({
    id: char.id,
    name: char.name,
  ...
  const mentionElements = (elements || []).map(elem => ({
    id: elem.id,
    name: elem.name,
  ...
  console.log('📝 Mention Debug:', {
    charactersCount: mentionCharacters.length,
    elementsCount: mentionElements.length,
  ...
  const handlePromptChange = (value: string) => {
    if (activePanel !== null) {
      // 멘션 태그를 실제 텍스트로 변환하여 AI 프롬프트용 저장
      const processedPrompt = processMentions(value, mentionCharacters, mentionElements);
      ...
      const enhancedValue = processedPrompt.includes('[ASPECT]') 
        ? processedPrompt.replace('[ASPECT]', aspectRatioInfo)
        : processedPrompt;
      ...
      // 캐릭터 이름 감지 및 자동완성 제안
      detectCharacterMentions(processedPrompt);
    }
  };
          ...
          <div className="space-y-4 h-full flex flex-col">
            ...
            <div className="flex-1 flex flex-col">
              ...
              <MentionTextArea
                ...
                characters={mentionCharacters}
                elements={mentionElements}
                placeholder="캐릭터와 배경만 설명하세요. @를 입력해서 캐릭터나 요소를 선택할 수 있습니다. 예: '@캐릭터:이름이 학교 교실에서 창밖을 바라보고 있어'"
