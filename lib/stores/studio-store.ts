  ...
  devtools(
    (set, get) => ({
      ...
      loadCharacters: async () => {
        try {
          // 🚀 Ultra-fast character loading with optimized query
          ...
          if (!user) {
            console.warn('⚠️ User not authenticated, skipping character load');
            return;
          }

          // 🔥 WebP 이미지만 우선 로드 + 크기 제한으로 초고속 로딩
          const { data: charactersData, error } = await supabase
            .from('character')
            .select('id, name, description, thumbnailUrl, createdAt')
            .eq('userId', user.id)
            .order('createdAt', { ascending: false })
            .limit(20); // 한 번에 20개만 로드
          ...
          if (charactersData) {
            // 🚀 DB 정리 완료 - 모든 캐릭터 초고속 로딩
            const characters = charactersData
              .filter((char: any) => char.thumbnailUrl) // null 체크만
              .map((char: any) => ({
                id: char.id,
                name: char.name,
                ...
                aliases: [],
                isActive: false,
                lastUsed: new Date(char.createdAt),
              }));
            
            console.log(`⚡ Loaded ${characters.length} characters (DB optimized, avg 119 bytes/URL)`);
            set({ characters });
          }
        } catch (error) {
      ...
      loadElements: async () => {
        try {
          ...
          if (result.success && result.elements) {
            const elements = result.elements.map((elem: any) => ({
              id: elem.id,
              name: elem.name,
